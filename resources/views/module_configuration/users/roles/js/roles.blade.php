<script>
    $(function () {
        var tabla_admin_roles = $("#role_table");

        function inicializar() {
            tabla_admin_roles.DataTable({
                'ajax': {
                    "url": '{{url("/admin/config/privilegios/roles")}}',
                    "type": "GET",
                    dataSrc: '',
                },
                'columns': [
                    ...
                    {data: 'description'},
                    {
                        render: function (data, type, row) {
                            var ruta_edit = "{{url('/admin/config/privilegios/roles/')}}/" + row.id + "/edit";
                            var ruta_destroy = "{{url('/admin/config/privilegios/roles/')}}/" + row.id;
                            var btn_editar = "";
                            ...
                            @permission('privilegios_eliminar')
                                btn_eliminar += '<form method="POST" class="frm-desactivar eliminar-rol" action="' + ruta_destroy + '">';
        ...
        $('body').on('click', 'tbody .eliminar-rol', function (e) {
                ...
                $.ajax({
                    ...
                    success: function (respuesta) {
                        if (respuesta.success) {
                            tabla_admin_roles.DataTable().ajax.reload();
