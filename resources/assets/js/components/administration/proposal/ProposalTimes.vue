     ...
     <div class="col-xs-8 col-xs-offset-3">
         ...
         <input type="radio" v-model="inputType" value="B"> LIBRE
          ...
          <div class="form-group col-xs-10 col-xs-offset-1" v-if="inputType == 'A'">
            ...
            <select v-model="modelTimeId"  class="form-control" name="timeId" id="timeId">
                <option v-for="(item,index) in times" :value="item.timeId" > {{item.timeName}}</option>
            </select>
          </div>

           <div class="form-group col-xs-10 col-xs-offset-1" v-if="inputType == 'B'">
            <label for="timeName">TIME FRAME</label>
            <input v-model="modelTimeName" type="text" class="form-control" id="timeName" name="timeName"  autocomplete="off">
          </div>
          ...
          <h4><b>Time Frame</b></h4>
           <ul>
            <li v-for="(item,index) in timesList">
              ...
              <a @click="deleteTime(++index)" class="btn btn-danger btn-xs" data-toggle="tooltip" data-placement="top" title="Eliminar">
     ...
     addRow: function() {
        ...
        let timeSelected = this.times.filter(filtrarPorID);
            //AGREGAR A ITEMLIST
              ...
              this.timesList.push({timeId:timeSelected[0].timeId,timeName:timeSelected[0].timeName,});
           } //end of the errors.length
         } else{ //end of de inputType A

               //VALIDATIONS
               if (!this.modelTimeName) 
