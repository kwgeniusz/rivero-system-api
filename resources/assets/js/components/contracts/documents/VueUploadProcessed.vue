    ...
    <div class="table-responsive">
      <table class="table table-hover">
        ...
        <tbody>
          ...
          <tr v-for="(file, index) in files2" :key="file.id">
            <td>{{index}}</td>
            <td>
            ...
            <td>
              <div class="btn-group">
                ...
                <div class="dropdown-menu">
                  ...
                  <a :class="{'dropdown-item': true, disabled: !file.active}" href="#" @click.prevent="file.active ? $refs.upload2.update(file, {error: 'cancel'}) : false">Cancelar</a>

                  <a class="dropdown-item" href="#" v-if="file.active" @click.prevent="$refs.upload2.update(file, {active: false})">Abortar</a>
                  <a class="dropdown-item" href="#" v-else-if="file.error && file.error !== 'compressing' && $refs.upload2.features.html5" @click.prevent="$refs.upload2.update(file, {active: true, error: '', progress: '0.00'})">Reintentar carga</a>
                  <a :class="{'dropdown-item': true, disabled: file.success || file.error === 'compressing'}" href="#" v-else @click.prevent="file.success || file.error === 'compressing' ? false : $refs.upload2.update(file, {active: true})">Cargar</a>

                  <div class="dropdown-divider"></div>
                  <a class="dropdown-item" href="#" @click.prevent="$refs.upload2.remove(file)">Remover</a>
                </div>
              </div>
            </td>
          </tr>
 ...
 props: {
  ...
  watch: {
    'editFile.show'(newValue, oldValue) {
      ...
      if (!newValue && oldValue) {
        this.$refs.upload2.update(this.editFile.id, { error: this.editFile.error || '' })
      }
      if (newValue) {
  ...
  methods: {
      ...
      if (newFile && !oldFile) {
        ...
        if (newFile.file && newFile.type.substr(0, 6) === 'image/' && this.autoCompress > 0 && this.autoCompress < newFile.size) {
          ...
          imageCompressor.compress(newFile.file)
            .then((file) => {
              this.$refs.upload2.update(newFile, { error: '', file, size: file.size, type: file.type })
            })
            .catch((err) => {
              this.$refs.upload2.update(newFile, { error: err.message || 'compress' })
            })
        }
      }
    ...
    inputFile2(newFile, oldFile) {
      if (newFile && oldFile) {
        ...
        if (newFile.success && !oldFile.success) {
                this.$emit('insert')
                this.$refs.upload2.remove(newFile) 
        }
      }
      if (!newFile && oldFile) {
        // remove
    ...
    onAddFolader() {
      ...
      let input = this.$refs.upload2.$el.querySelector('input')
