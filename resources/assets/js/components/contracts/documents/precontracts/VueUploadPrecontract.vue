...
<div class="example-full">
  ...
  <div class="upload1" v-show="!isOption">
    <div class="table-responsive">
      <table class="table table-hover">
        ...
        <tbody>
          ...
          <tr v-for="(file, index) in files1" :key="file.id">
            <td>{{index}}</td>
            <td>
            ...
            <td>
              <div class="btn-group">
                ...
                <div class="dropdown-menu">
                  ...
                  <a :class="{'dropdown-item': true, disabled: !file.active}" href="#" @click.prevent="file.active ? $refs.upload1.update(file, {error: 'cancel'}) : false">Cancelar</a>

                  <a class="dropdown-item" href="#" v-if="file.active" @click.prevent="$refs.upload1.update(file, {active: false})">Abortar</a>
                  <a class="dropdown-item" href="#" v-else-if="file.error && file.error !== 'compressing' && $refs.upload1.features.html5" @click.prevent="$refs.upload1.update(file, {active: true, error: '', progress: '0.00'})">Reintentar carga</a>
                  <a :class="{'dropdown-item': true, disabled: file.success || file.error === 'compressing'}" href="#" v-else @click.prevent="file.success || file.error === 'compressing' ? false : $refs.upload1.update(file, {active: true})">Cargar</a>

                  <div class="dropdown-divider"></div>
                  <a class="dropdown-item" href="#" @click.prevent="$refs.upload1.remove(file)">Remover</a>
                </div>
              </div>
            </td>
          </tr>
...
export default {
  ...
  data:function () {
    return {
      files1: [],
      accept: '',
     extensions: 'gif,jpg,jpeg,png,webp,pdf,doc,docx,ppt,pptx,xls,xlsx,mp4,mov,mp3,dwg,dxf,dwf,bin,bak,HEIC,heic, heif,zip,tif,skp,dgn,dsd,ctb,pln,bpn,gsm,lcf,rar,sdb,pro,mcdx,kmz,kml,gmw,csv,txt,fbx,ls8,ls9,ls10,bimx,shx,SHX',                       
      ...
      thread: 3,
      name: 'file1',
      postAction: '/precontracts/'+this.precontractId+'/files',
      putAction: '/precontracts/'+this.precontractId+'/files',
      headers: {
 ...
 props: {
  ...
  watch: {
    'editFile.show'(newValue, oldValue) {
      ...
      if (!newValue && oldValue) {
        this.$refs.upload1.update(this.editFile.id, { error: this.editFile.error || '' })
      }
      if (newValue) {
  ...
  methods: {
      ...
      if (newFile && !oldFile) {
        ...
        if (newFile.file && newFile.type.substr(0, 6) === 'image/' && this.autoCompress > 0 && this.autoCompress < newFile.size) {
          ...
          imageCompressor.compress(newFile.file)
            .then((file) => {
              this.$refs.upload1.update(newFile, { error: '', file, size: file.size, type: file.type })
            })
            .catch((err) => {
              this.$refs.upload1.update(newFile, { error: err.message || 'compress' })
            })
        }
      }
    ...
    inputFile1(newFile, oldFile) {
      if (newFile && oldFile) {
        ...
        if (newFile.success && !oldFile.success) {
                this.$emit('insert')
                this.$refs.upload1.remove(newFile) 
        }
      }
      if (!newFile && oldFile) {
        // remove
    ...
    // add folader
    onAddFolader() {
      if (!this.$refs.upload1.features.directory) {
        this.alert('Your browser does not support')
        return
      }
      let input = this.$refs.upload1.$el.querySelector('input')
