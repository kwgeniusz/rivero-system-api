<template>
	<div class="col-md-12">
        <h2 class="text-uppercase">Pre vacaciones</h2>
        ...
        <div class="panel panel-default">
            ...
            <div class="table-responsive text-center">
                <table class="table table-striped table-bordered text-center">
                    ...
                    <tbody v-if="objPreVacation.length > 0">
                        <tr v-for="(preVacation, index) in objPreVacation" :key="preVacation.hrpayrollControlId">
                            <td >{{index + 1}}</td>
                            <td class="form-inline">
                                <p class="text-left">
                                    {{preVacation.countryName}}
                                </p>
                            </td>
                            <td > 
                                <p class="text-left">
                                    {{preVacation.companyName}} 
                                </p>
                            </td>
                            <td>
                                <p class="text-left">
                                    {{preVacation.payrollTypeName}}
                                </p>
                            </td>
                            <td>
                                {{preVacation.year}}
                            ...
                            <td>
                                <p class="text-left">
                                    {{preVacation.payrollName}} 
                                </p>
                            </td>
                            <td>
                                <!-- <loading v-if="loading == 1"></loading> -->
                                <button v-if="loading === false" v-on:click="process(preVacation.hrpayrollControlId)" class="btn btn-sm btn-primary"><i class="fa fa-edit"></i>Calcular</button>  
                                <button v-else disabled="disabled" class="btn btn-sm btn-primary"><i class="fa fa-edit"></i>Calcular</button>  
                                <button v-on:click="printPreVacation( preVacation.year, preVacation.payrollNumber,preVacation.payrollTypeId)" class="btn btn-sm btn-info"><i class="fa fa-print"></i> Imprimir</button>  
                                <button v-on:click="upgradeVacation(preVacation.year, preVacation.payrollNumber,preVacation.payrollTypeId)" class="btn btn-sm btn-warning"><i class="fa fa-edit"></i>Actualizar</button>  
                                <button v-on:click="deletePreVacation(preVacation.hrpayrollControlId)" class="btn btn-sm btn-danger"><i class="fa fa-times-circle"></i></button>  
                            </td>
                        </tr>
                    </tbody>
                    <tbody v-else>
...
export default {
	...
	methods: {
        getListPreVacation(){
            axios.get('pre-vacations/list').then( res => {
                res.data.length > 0 ? this.lengths = false : this.lengths = true
                this.objPreVacation = res.data 
                // console.log(this.objPreVacation)
            })
        },
        deletePreVacation(id){
            // console.log(id)
            if (confirm("Delete?") ){      
                axios.delete(`pre-vacations/${id}`).then((res)=>{
                    // console.log(res)
                    this.getListPreVacation()
                })
        ...
        upgradeVacation(year, payrollNumber, payrollTypeId){
            this.loading = true
            const URL = `vacations/upgrade/${year}/${payrollNumber}/${payrollTypeId}`
            axios.get(URL).then((res) => {
                // let transProcessed = res.data.transProcessed
                // console.log(res)
                if (res.statusText === 'OK') {
        ...
        printPreVacation(year, payrollNumber,payrollTypeId){
            const URL = `pre-vacations/print/${year}/${payrollNumber}/${payrollTypeId}`
