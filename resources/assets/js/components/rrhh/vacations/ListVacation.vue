<template>
	<div class="col-md-12">
        <h2 class="text-uppercase">Vacaciones</h2>
        <div class="panel panel-default" >
            <div class="container">
                <div class="row">
                    ...
                    <div class="col-md-2">
                        <label for="currency" class="form-group" v-text="'Moneda'"></label>&nbsp;&nbsp;&nbsp;
                        <label>
                    ...
                    <div class="col-md-1">
                        <label>
                            <button v-on:click="getListVacation()" class="btn btn-sm btn-info"><i class="fas fa-sync-alt"></i></button>  
                        </label>
                    </div>
                    ...
                    <div class="col-md-2">
                        <div>
                            <label for="">
                                Periodo Desde: <input v-model="fromDate" type="date" required="required" class="form-control">
                            </label>
                        </div>
                    </div>
                    <div class="col-md-2">
                        <div>
                            <label for="">
                                Periodo Hasta: <input v-model="toDate" type="date" required="required" class="form-control">
                            </label>
                        </div>
                    </div>
                    <div class="col-md-1">
                        <label for=""> &nbsp;</label>
                        <button type="button" v-on:click="printVacationByEmployee()" class="btn btn-sm btn-info"><i class="fa fa-print"></i> Imprimir</button>  
                    </div>
                    <div class="col-md-2 col-md-offset-4">
                        <div>
                            <label for="">
                                Fecha Salida vac.: <input v-model="startVacation" type="date" required="required" class="form-control">
                            </label>
                        </div>
                    </div>
                    <div class="col-md-2">
                        <div>
                            <label for="">
                                Fecha retorno vac.: <input v-model="endVacation" type="date" required="required" class="form-control">
                            </label>
                        </div>
                    </div>
                </div>
        ...
        <div class="panel panel-default">
            ...
            <div class="table-responsive text-center">
                <table class="table table-striped table-bordered text-center">
                    ...
                    <tbody v-if="objVacation.length > 0">
                        <tr v-for="(vacation, index) in objVacation" :key="vacation.hrpayrollControlId">
                            <td >{{index + 1}}</td>
                            ...
                            <td>
                                <p class="text-left">
                                    {{vacation.payrollName}} 
                                </p>
                            </td>
                            <td>
                                <!-- <loading v-if="loading == 1"></loading> -->
                                <button type="button" v-on:click="printVacation( vacation.year, vacation.payrollNumber,vacation.payrollTypeId)" class="btn btn-sm btn-info"><i class="fa fa-print"></i> Imprimir</button>  
                    ...
                    <tbody v-else>
                        <tr>
                            <td v-if="this.lengths" colspan="9">
...
export default {
	...
	methods: {
        getListVacation(){
            axios.get('vacations/list').then( res => {
                // console.log(res)
                // return
                res.data.payrollHistory.length > 0 ? this.lengths = false : this.lengths = true
                this.objVacation = res.data.payrollHistory
                console.log(this.objVacation)
            })
        },
        getComboEmployees(){
            const URL1 = `combo-staff/${0}/${0}`
            // obtiene el personal 
        ...
        printVacationByEmployee(){

            // obtengo el periodo
            const URL2 = `get-payroll-number-vacation/${this.fromDate}/${this.toDate}`
            ...
            axios.get(URL2).then( res => {
                ...
                let payrollNumber = res.data.payrollNumber
                let payrollTypeId = res.data.payrollTypeId
                let payrollCategory = res.data.payrollCategory
                const selecCurrency1 = this.currency

                const URL = `vacations-employees/print/${year}/${payrollNumber}/${payrollTypeId}/${payrollCategory}/${this.selectStaff}` 
                ...
                axios.get(URL).then( res => {
                    ...
                    employmentDate = this.formatDate2(employmentDate)
                    ...
                    function centerText(text) {
                        ...
                        if(text.length < 20){
                            ...
                            return numCenter
                        } else { // si es rivero visual group o otras con nombres largos
                            return numCenter
                        }
                    }
                    ...
                    function crearPDF(imgData){
                        ...
                        if (companyId == 4) {
                            ...
                            let yearYYYY = dataTime.getFullYear()
                            doc.text( `© Copyright ${yearYYYY} Rivero Visual Group - All rights reserved `, 210, 760 );
                            doc.setFontType("normal");
                        } 
                        if (companyId == 5) {
                            doc.addImage(icoPhone, 'JPEG', 260, 92, 9, 9)
                            ...
                            let dataTime = new Date()
                            let yearYYYY = dataTime.getFullYear()
                            doc.text( `© Copyright ${yearYYYY} JD Rivero Global - All rights reserved `, 215, 760 );
                            doc.setFontType("normal");
                        }
                        ...
                        doc.text( `Con el pago de estos montos la empresa cancela todo lo relacionado con vacaciones del período que se menciona en este mismo recibo y por tal razón nada`, 30, n + 100 );
                        doc.text( `tendré que reclamar a futuro por este concepto.`, 30, n + 110 );
        ...
        printVacation(year, payrollNumber,payrollTypeId){
            const URL = `vacations/print/${year}/${payrollNumber}/${payrollTypeId}/vacation`
            const selecCurrency1 = this.currency
